...
class Server {
    ...
    {
        $this->config = $config;
        $this->ds_storage = new StackableResolver($config);
        ...
        if(!class_exists('swoole_server')) {
            $this->ds_error(E_USER_ERROR, 'Swoole extension or function not found.', __FILE__, __LINE__);
        }
        
        $this->swoole_server = new \swoole_server($config['ip'], $config['port'], SWOOLE_PROCESS, SWOOLE_SOCK_UDP);
        $this->swoole_server->set([
            'work_num' => $config['work_num']
        ]);
        
        $this->swoole_server->on('Packet', function (\swoole_server $serv, $data, $addr) {
            $response = $this->ds_handle_query($data);

            $serv->sendto($addr['address'], $addr['port'], $response);
        });
    }
